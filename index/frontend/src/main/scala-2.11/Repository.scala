...
object Repository extends JSApp {
  ...
  def previewRepository(click: Event) : Boolean = {
    ...
    requests("add-repo-cat")   = jQuery("select#add-repo-cat option:selected").value().toString
    requests("add-repo-desc")  = jQuery("input#add-repo-desc").value().toString
    requests("add-repo-proj")  = jQuery("input#add-repo-proj").value().toString
          ...
          if (results.contains("status")) {
            results("status") match {
              case "duplicated" => {
                ...
                jQuery("select#add-repo-cat").value(results("add-repo-cat").toString)
                jQuery("input#add-repo-proj").value(results("add-repo-proj").toString)
                jQuery("input#add-repo-logo").value(results("add-repo-logo").toString)
              }
            }
          }
  ...
  def submitRepository() : Boolean = {
    ...
    val xhr = new XMLHttpRequest()
    // TODO : Once a page is tagged as UPDATE, it will never change. Try refactor.
    var mode = "submit"
    if (update) {
      mode = "update"
    }
