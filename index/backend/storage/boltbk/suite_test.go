...
func (s *BackendSuite) BasicCRUD(c *C) {
    ...
    c.Assert(IsBadParameter(err), Equals, true, Commentf("%#v", err))
    _, _, err = s.B.GetValAndTTL([]string{"a"}, "b")
    c.Assert(IsBadParameter(err), Equals, true, Commentf("%#v", err))
    _, err = s.B.GetVal([]string{"a", "b"}, "x")
    c.Assert(IsNotFound(err), Equals, true, Commentf("%#v", err))
    ...
    c.Assert(IsNotFound(s.B.DeleteKey([]string{"a", "b"}, "bkey")), Equals, true)
...
func (s *BackendSuite) CompareAndSwap(c *C) {
    ...
    c.Assert(IsAlreadyExists(err), Equals, true, Commentf("%#v", err))

    _, err = s.B.CompareAndSwap([]string{"a", "b"}, "bkey", []byte("val2"), 0, []byte("abcd"))
    c.Assert(IsCompareFailed(err), Equals, true, Commentf("%#v", err))
...
func (s *BackendSuite) Locking(c *C) {
    ...
    c.Assert(s.B.ReleaseLock(tok1), IsNil)
    err = s.B.ReleaseLock(tok1)
    c.Assert(IsNotFound(err), Equals, true, Commentf("%T", err))
}
