...
func serveSync(w http.ResponseWriter, r *http.Request, fsRoot, fileName string) {
    fs := http.Dir(fsRoot)

    f, err := fs.Open(fmt.Sprintf("%s.pcsync",fileName))
    if err != nil {
...
func PackageSync(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
    ...
    serveSync(w, r, api.FSPackageRootSync, mName)
}
